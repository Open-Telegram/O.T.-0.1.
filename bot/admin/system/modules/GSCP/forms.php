...
class TRadioGroup extends TFListControl {
  ...
  public function AddItems($vals, $texts, $selval="") {
    $i = 0;
    foreach ($vals as $val) {
      if (isset($texts[$i])) $s = $texts[$i]; else $s = "";
...
abstract class TCustomForm extends TBlockControl {
  ...
  public function AddEdit($name, $size=0, $value="") {
    $ctrl = $this->AddCustomInput($name, 'text', $this);
    $ctrl->SetSize($size);
    $this->SetDefVal($ctrl,$name,$value);
    return $ctrl;
  }
  public function AddPassword($name, $size=0, $value="") {
    $ctrl = $this->AddCustomInput($name, 'password', $this);
    $ctrl->SetSize($size);
  ...
  public function AddHidden($name, $value) {
    $ctrl = $this->AddCustomInput($name, 'hidden', $this);
    //$ctrl->SetAttr("value",$value);
    $this->SetDefVal($ctrl,$name,$value);
    return $ctrl;
  }
  public function AddButton($name, $caption) {
    $ctrl = $this->AddCustomInput($name, 'button', $this);
    $ctrl->SetAttr("value",$caption);
    return $ctrl;
  }
  public function AddSubmit($name, $caption="") {
    $ctrl = $this->AddCustomInput($name, 'submit', $this);
    $ctrl->SetAttr("value",$caption);
    return $ctrl;
  }
  public function AddReset($name, $caption="") {
    $ctrl = $this->AddCustomInput($name, 'reset', $this);
    $ctrl->SetAttr("value",$caption);
    return $ctrl;
  }
  ...
  public function AddCheckBox($name, $checked = false, $text = "") {
    $ctrl = $this->AddCustomInput($name, 'checkbox', $this);
    if (!is_null($checked)) {
  ...
  public function AddRadioButton($name, $value = "", $checked = false) {
    $ctrl = $this->AddCustomInput($name, 'radio', $this);
    if ($value != "") $ctrl->SetAttr("value", $value);
    if ($checked) $ctrl->SetAttr("checked","checked");
    return $ctrl;
  }
...
//echo "Reset data<br />";
  ...
  public function prn() {echo "<pre>"; print_r($this->adata); echo "</pre>";}
}
