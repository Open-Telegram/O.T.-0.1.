...
require_once("img.php");
 ...
 class TNewsH extends TDBBase {
   ...
   public function GetArchivesList($mon, $year, $limit = 0) {
     if ($limit>0) $llim = " limit 0, $limit "; 
     if ($mon>0) $mlim = " and (DATE_FORMAT($this->datefield,\"%m\")=$mon) ";
   ...
   public function GetArchivesContents($aurl, $yparam, $mparam) {
     ...
     $row = $this->db->FetchObject();  //skip last (?)
     $oyear = 0;
     $omonth = 0;
     $mcount = 0;
     ...
     while ($row = $this->db->FetchObject()) {
       ...
       if ($oyear !== $cyear) {
         ...
         $trow = $ret->CreateRow();
         $nycell = $trow->CreateCell('th',3)->content = $row->YY;
         $mcount = 0;
       } 
       if ($omonth !== $cmonth) {
