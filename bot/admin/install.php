...
<form method="POST">
	...
	<input type="text" required name="telegramApi">Введите телеграм апи код.<hr>

	<input type="text" required name="DB_log">Введите логин БД<br>
	<input type="text" required name="DB_pass">Введите пароль БД<br>
	<input type="text" required name="DB_host">Хост. Обычно - localhost<br>
	<input type="text" required name="DB_name">Название базы данных<hr>

	<input type="text" value="<?php echo RandomString(). rand(rand(1000,1000000), rand(10000000,10000000000)) . RandomString(); ?>" required name="key_password">Ваш ключ проета - сохраните его!<hr>
...
<?php else: ?>
<?php
		$db = mysqli_connect($_POST['DB_host'],$_POST['DB_log'],$_POST['DB_pass'],$_POST['DB_name']) or die('<script>alert("Не удалось подключиться к базе данных");window.location = window.location.href;</script>');

		getSQL($db);
		$botAPI = $_POST['telegramApi'];//Удалить эту строку после подключения бота.
		$url = "https://".$_SERVER['SERVER_NAME']."?id=$botAPI";//И эту
